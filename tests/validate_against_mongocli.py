...
Prerequisites:
    - mongocli installed and configured (https://www.mongodb.com/docs/mongocli/)
    - Access to an Ops Manager instance
    - Environment variables or mongocli profile configured
...
Usage:
    ...
    python tests/validate_against_mongocli.py --endpoint clusters
...
def run_mongocli(args: List[str]) -> Dict[str, Any]:
...
def compare_results(
    ...
    if isinstance(py_result, dict) and isinstance(cli_result, dict):
        # Normalize CLI keys to snake_case for comparison
        cli_normalized = normalize_keys(cli_result, to_snake=True)

        all_keys = set(py_result.keys()) | set(cli_normalized.keys())
        for key in all_keys:
            ...
            elif key not in cli_normalized:
                differences.append(f"Missing in CLI: {key_path}")
            else:
                differences.extend(
                    compare_results(py_result[key], cli_normalized[key], key_path, ignore_fields)
                )
...
def validate_clusters(client: OpsManagerClient, project_id: str) -> bool:
    """Validate clusters endpoint."""
    print("\n=== Validating Clusters ===")

    # Get from Python library (raw dict)
    py_clusters = client.clusters.list(project_id=project_id, as_obj=False)

    # Get from mongocli
    cli_result = run_mongocli(["clusters", "list", "--projectId", project_id])
    cli_clusters = cli_result.get("results", cli_result)

    print(f"Python returned {len(py_clusters)} clusters")
    print(f"mongocli returned {len(cli_clusters)} clusters")

    # Compare
    differences = compare_results(py_clusters, cli_clusters)
    ...
    print("PASS: Clusters match!")
    return True


def validate_hosts(client: OpsManagerClient, project_id: str) -> bool:
    ...
    # Get from mongocli
    cli_result = run_mongocli(["processes", "list", "--projectId", project_id])
...
def validate_alerts(client: OpsManagerClient, project_id: str) -> bool:
    ...
    # Get from mongocli
    cli_result = run_mongocli(["alerts", "list", "--projectId", project_id])
...
def main():
    ...
    parser.add_argument("--endpoint", choices=["clusters", "hosts", "alerts", "all"],
    ...
    if not all([args.base_url, args.public_key, args.private_key, args.project_id]):
        print("Error: Missing required configuration.")
        print("Set environment variables or use command-line arguments:")
        print("  OM_BASE_URL, OM_PUBLIC_KEY, OM_PRIVATE_KEY, OM_PROJECT_ID")
        sys.exit(1)
    ...
    try:
        if args.endpoint in ("clusters", "all"):
            results["clusters"] = validate_clusters(client, args.project_id)

        if args.endpoint in ("hosts", "all"):
            results["hosts"] = validate_hosts(client, args.project_id)

        if args.endpoint in ("alerts", "all"):
            results["alerts"] = validate_alerts(client, args.project_id)
